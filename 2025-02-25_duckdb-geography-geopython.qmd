...
tab = con.read_parquet(
  ...
  "st_extent_agg(geometry).st_aswkb() as geom",
  "filename"
).to_arrow_table()

geopandas.GeoSeries.from_wkb(tab["geom"]).plot(edgecolor="black", facecolor="none")
```
...
result = sindex.query(zipcodes_df, predicate="contains")
counts = pd.Index(pd.Series(chunk.index).iloc[result[0]]).value_counts(sort=False)
```
...
Let's count these by zipcode ~using PostGIS~
...
...by using S2 cells as something that DuckDB knows how to optimize into a join.
...
con.table("zipcodes_original").limit(1)
```
...
```{python}
con.sql("""
  CREATE OR REPLACE VIEW zipcodes AS
  SELECT
